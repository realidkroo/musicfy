...
import com.example.musicfy.db.entities.AlbumArtistMap
import com.example.musicfy.db.entities.AlbumEntity
import com.example.musicfy.db.entities.AlbumWithSongs
...
import com.example.musicfy.db.entities.EventWithSong
...
interface DatabaseDao {
    ...
    @Query(
        ...
        WHERE playlist_event.timestamp > :fromTimeStamp AND playlist_event.timestamp <= :toTimeStamp
        GROUP BY playlist_event.playlistId
        ORDER BY count(playlist_event.id) DESC
        LIMIT :limit
        OFFSET :offset
        """
    )
    fun mostPlayedPlaylists(
        fromTimeStamp: Long,
        limit: Int = 6,
        offset: Int = 0,
        toTimeStamp: Long? = LocalDateTime.now().toInstant(ZoneOffset.UTC).toEpochMilli(),
    ...
    @Insert(onConflict = OnConflictStrategy.IGNORE)
    fun insert(event: PlaylistEvent)
