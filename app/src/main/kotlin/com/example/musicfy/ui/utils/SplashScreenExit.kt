 ...
 * background, then the black splash view fades away to reveal the
 * already-composed app content underneath — reading as the app fading in
 * from black.
 *
 * The splash is held a little past "content ready" (`MinHoldMillis`) because
 * that signal fires on first draw, before cold-start work (DI, DataStore
 * reads, initial composition) has actually settled — starting the exit
 * animation while the main thread is still busy is what caused it to skip
 * frames.
 */
